...
!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        paginationShowPageGo: true,//20170822 王超 扩展select跳转
        paginationUseBSSelect:false,//20170822 王超 扩展select跳转 启用BoostrapSelect
    ...
    BootstrapTable.LOCALES['en-US'] = BootstrapTable.LOCALES['en'] = {
        ...
        formatShowingRows: function (pageFrom, pageTo, totalRows,totalPages) {
            return sprintf('Showing %s to %s of %s rows %s pages', pageFrom, pageTo, totalRows,totalPages);
        },
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        if (!this.options.onlyInfoPagination) {
            ...
            //20170822 王超
                if(this.options.paginationShowPageGo){
                    html.push('跳转至  <li class="bootstrap-table-page-go">',
                        '<input class="bootstrap-table-page-go-text" type="text" id="goPage" style="width: 26px" value="'+this.options.pageNumber+'">');

                    html.push('</input>','</li> 页 ');
                    html.push('<li class="bootstrap-table-page-go-button"><input class="bootstrap-table-page-go-button1" type="button" value="确定"></li>');
                }
        ...
        //20170822 王超 扩展select跳转 启用BoostrapSelect
        ...
        if (!this.options.onlyInfoPagination) {
            ...
            //20170822 王超 扩展select跳转
            //var $pagegotext = this.$pagination.find('.bootstrap-table-page-go').find('input');
            $pagego = this.$pagination.find('.bootstrap-table-page-go-button').find('input');
            ...
            //20170822 王超 扩展select跳转
            // this.options.pageNumber = $pagegotext.val();
            // debugger;
            if($pagego){
    ...
    BootstrapTable.prototype.getRowsByUniqueId = function (id) {
        var uniqueId = this.options.uniqueId,
            len = this.options.data.length,
            dataRow = null,
            i, row, rowUniqueId;
    ...
    BootstrapTable.prototype.hideRow = function (params) {
        console.info('进入Table中getAllSelections');
        if (!params.hasOwnProperty('index') || !params.hasOwnProperty('uniqueId')) {
            return;
        }
    ...
    BootstrapTable.prototype.onPageGo = function (event) {
        var goNumber = $('.bootstrap-table-page-go').find('input').val();

        if(+goNumber <= 0){
            return;
        }

        if(+goNumber > this.options.totalPages){
            return;
        }

        if (this.options.pageNumber === +goNumber) {
            return;
        }
        this.options.pageNumber = +goNumber;
        this.updatePagination(event);
        return false;
    };
